...
function [successOutput] = wallAdaptic_disc_corr(XPiers_filtered, YPiers_filtered, ZPiers_filtered, XSpandrels_filtered, YSpandrels_filtered, ZSpandrels_filtered, XNodes_filtered, YNodes_filtered, ZNodes_filtered, X_face, Y_face)
    ...
    for i = 1:length(face_values)
        ...
        for i = 1:numel(groups)
            ...
            if numel(group_indices) == 4 % Check if group has four nodes
                ...
                meshmat{group_indices(3), 6} = [meshmat{group_indices(2), 6}];
            elseif numel(groups{i}) == 6 % Check if the group has six nodes
                % Replace node names for groups of six
                meshmat{groups{i}(2), 6} = meshmat{groups{i}(3), 6}; % Replace first with third
                meshmat{groups{i}(4), 6} = meshmat{groups{i}(5), 6}; % Replace second with fourth
            elseif numel(groups{i}) == 8 % Check if the group has eight nodes
                % Replace node names for groups of eight
                meshmat{groups{i}(1), 6} = meshmat{groups{i}(8), 6}; % Replace first with eighth
                meshmat{groups{i}(2), 6} = meshmat{groups{i}(3), 6}; % Replace second with third
                meshmat{groups{i}(4), 6} = meshmat{groups{i}(5), 6}; % Replace fourth with fifth
                meshmat{groups{i}(6), 6} = meshmat{groups{i}(7), 6}; % Replace sixth with seventh
            end
        end
