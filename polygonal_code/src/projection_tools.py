...
def undistort_img(img, K,k):
    distCoef = np.array([k[0], k[1], 0., 0., k[2]], dtype=np.float32)
    h,w = img.shape[:2]
    newcameramtx, roi = cv2.getOptimalNewCameraMatrix(K, distCoef, (w,h), 1, (w,h))
    u_img = cv2.undistort(img, K, distCoef, None, newcameramtx)
    plt.imshow(u_img)
    plt.show()

#Function to undistort image
def undistort_img_fish(img, K,k):
    distCoef = np.array([k[0], k[1], k[2], k[3]], dtype=np.float32)
    h,w = img.shape[:2]
    newK = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(K, distCoef, (w,h),np.eye(3))
    u_img = cv2.fisheye.undistortImage(img, K, distCoef, None, newK)
    plt.imshow(u_img)
    plt.show()
...
def undistort_points(points2d, K, k_dist):
    ...
    distCoef = np.array([k_dist[0], k_dist[1], 0., 0., k_dist[2]], dtype=np.float32) 
...
#Function to undistort points 
def undistort_points_fish(points2d, K, k_dist):
    """
    Find image coordinates when undistorted
    ...
    distCoef = np.array([k_dist[0], k_dist[1], k_dist[2], k_dist[3]], dtype=np.float32) 
    
    result = np.squeeze(cv2.fisheye.undistortPoints(points2d_, K, distCoef))
    if len(result.shape) == 1:  # only a single point
        result = np.expand_dims(result, 0)
...
def camera_matrices(intrinsic, poses, return_Rt=False):
    ...
    for ii in intrinsic:
        ...
        f = float(intrinsic[ii]["pxFocalLength"]) #EMA19
        ...
        K[ii]= np.array([[f,0.,px],
                         [0.,f,py],
                         [0.,0.,1.]])
        P_type[ii] = str(intrinsic[ii]['type'])
    ...
    if return_Rt:        
        return P, K, R, t, k_dist, P_type
    else:
        return P, K, k_dist, P_type
