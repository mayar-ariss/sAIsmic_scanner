...
from opening_segmentation import segment_opening_corners, segment_opening
...
def line_adjustor(x_lr,y_lr, smooth=1e-13):
    ...
    for i, x in enumerate(x_lr):
        u = np.array([x_lr[i][0], yu[i]])
        v = np.array([x_lr[i][0]+.2, yv[i]])
        X = np.array([x_lr[i][0], y_lr[i]])
...
def op_detector_2(data_folder, images_path, bound_box, two_views):
    ...
    #Segment opening corners
    images, corn_inference, overlayed_prediction_cracks = segment_opening_corners(data_folder, images_path, im_fold, ret_corn_inference = True)    
    ...
    for key in images:
        ...
        opening_information[key]['corner_centroids'] = corn_inference[key]
        
        #Drawing keypoints
        cent = opening_information[key]['corner_centroids']
        radius = int(overlayed_prediction_cracks[key].shape[0]/100)
        for c in cent:
            cv.circle(overlayed_prediction_cracks[key], (int(c[1]),int(c[0])), radius=radius, color=(0,0,255), thickness=-1)
        cv.imwrite('../results/' + data_folder + '/corners_' + key + '.jpg', overlayed_prediction_cracks[key])        
    
    ##SEMANTIC SEGMENTATION OPENINNG DETECTOR 
    prediction_bin_resized_openings, bboxes_dict, boxes_im_dict, labels_dict, label_op_f_dict = segment_opening(data_folder, images_path, im_fold, images, ret_op_inference=True)
    ...
    if bound_box=="region":
        ...
        for key in images:
            ...
            boxes_im = boxes_im_dict[key]
            labels = labels_dict[key]
            label_op_f = label_op_f_dict[key]

            bboxes = np.array(bboxes)
            bboxes[:,[0,1,2,3]] = bboxes[:,[1,0,3,2]]
            
            opening_information[key]["bboxes"] = (bboxes, labels, bound_box)
            cv.imwrite("../results/" + data_folder + "/boxes_" + key + ".jpg", boxes_im)
            
    return opening_information, prediction_bin_resized_openings
...
def corn_filt_1_2(box, building_corners, bound_box='region'):
    ...
    thresh_1 = 0.3*np.min([[np.linalg.norm(box_tr-box_tl)],[np.linalg.norm(box_tl-box_bl)], \
...
def corn_filt_4_5(face_open_corners_ord, facade):
    ...
    #Check if there is another region of pixels relative big in comparison to the maximum. If so, This also might be facade #!
    if (areas_cp[1]/max_area > 0.2): #Second biggest area is at least the 30% of the biggest#!
        print("There is a second facade in the image")
        ind_sec_area = np.where(areas==areas_cp[1])
        reg_sec_area = regions_op[ind_sec_area[0][0]]
        coords = reg_sec_area.coords
        main_facade[(coords[:,0], coords[:,1])] = True
...
def corn_filt_5(face_open_corners_ord, img, key, data_folder):
    ...
    if len(face_open_corners_ord)>0:
        cte = .2 #!
        ...
        #saving skeleton 
        cv.imwrite('../results/' + data_folder + '/skeleton_' + key + '.jpg', skeleton)
...
def op_detector_4p_1(opening_information, opening_labeled_corners, images_path, data_folder, two_views, cte_fil4 = .2):
    ...
    for key in opening_information:
        ...
        for c in face_open_corners_ord_al:
            ...
            ccc+=1
        cv.imwrite('../results/' + data_folder + '/corn_filtered_' + key + '.jpg', img)        
...
def op_detector_4p_2(opening_information, images_path, data_folder, bound_box, two_views, bin_facade_openings, cte_fil4 = .2):
    """
        Function to select just 4 points (4p) by opening according the ones nearest to the 
    ...
    for key in opening_information:
        ...
        for i, box in enumerate(opening_information[key]['bboxes'][0]):
            if opening_information[key]['bboxes'][1][i]=='building':
                ...
                #Filter1:Initial opening corners based on distance between full corners and boxes
        ...
        building_open_corners[key] = (face_open_corners_ord_al, main_facade, bin_facade_openings[key])
...
def main_op_detector(data_folder, images_path, bound_box, op_det_nn, two_views=False, cte_fil4 = .2):
    ...
    opening_information, binary_facade_openings = op_detector_2(data_folder, images_path, bound_box, two_views)
    opening_4points = op_detector_4p_2(opening_information, images_path, data_folder, bound_box, two_views, binary_facade_openings, cte_fil4 = cte_fil4)
