...
class Open:
    ...
    def build_open_ext(self, cte_ext=.03):
        poly = Part.makePolygon([tuple(self.o_vert[i]) for i in range(len(self.o_vert))])
        openn = Part.Face(poly)
        ...
        #print(normal)
        if np.linalg.norm(normal)!=0: #!BRepSweep_Translation::Constructor
            
            normal = normal * (1/(np.linalg.norm(normal)+1e-27))
            lenght = min(np.linalg.norm(P[0]-P[1]),np.linalg.norm(P[0]-P[2]),np.linalg.norm(P[1]-P[2]))
            #print("CTE EXT ", cte_ext)
            #print("NORMAL", normal)
            #print(P[0], P[1], P[2])
            ...
            print("An opening has repeated vertices. Check regularization thresholds")
            return None


#BULDING THE LOD3 model using structures and methods
def op_builder(data_folder, data_path, cte_ext = .03):
    ...
    for i, o in enumerate(openings_o):
        ...
        if O.open_free_ext is not None:
            opens.append(O)
