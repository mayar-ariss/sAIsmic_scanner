...
def fac_segmentation(data_folder, images_path, out_return=False):
    ...
    model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu'))) #eddychanged added , map_location=torch.device('cpu')
    model.train(False)
    model.eval()
    ...
    for key in images:
        ...
        cv.imwrite('../results/' + data_folder + '/' + key + '.jpg', images[key])
        cv.imwrite('../results/' + data_folder + '/' + key + '_overlayed1.jpg', overlayed_prediction1[key])
